...
from util.mixup import multilabel_mixup
from util.focalloss import FocalLoss
from engine_finetune_top1 import train_one_epoch, evaluate

def get_args_parser():
    parser = argparse.ArgumentParser('MAE fine-tuning for multi-label classification', add_help=False)
    parser.add_argument('--batch_size', default=4, type=int)
    parser.add_argument('--epochs', default=100, type=int)
    parser.add_argument('--accum_iter', default=1, type=int)

    parser.add_argument('--model', default='RETFound_mae', type=str)
    ...
    parser.add_argument('--finetune', default='/home/itaer2/zxy/shixi/retfound2/model/Retina/checkpoint-best.pth', type=str)
    parser.add_argument('--task', default='retina_gan2WeFoLoss', type=str)
    parser.add_argument('--global_pool', action='store_true')
    parser.set_defaults(global_pool=False)

    parser.add_argument('--data_path', default='/home/itaer2/zxy/shixi/retfound2/datasets/mutil/gan2', type=str)
...
def main(args, criterion):
    ...
    # ======== Step 1: 加载数据集和采样器 ======== #
    dataset_train, weighted_sampler_train = build_dataset('train', args)
    dataset_val, _ = build_dataset('val', args)
    dataset_test, _ = build_dataset('test', args)
    ...
    if True:  # args.distributed:
        ...
        if not args.eval:
            if args.distributed:
                ...
                # 非分布式时，使用 weighted_sampler（为 None 时 fallback 到 RandomSampler）
                sampler_train = weighted_sampler_train or torch.utils.data.RandomSampler(dataset_train)

            print("Sampler_train = %s" % str(sampler_train))

            if args.dist_eval:
                if len(dataset_val) % num_tasks != 0:
                    print('Warning: val set not divisible by #processes')
        ...
        if args.dist_eval:
            if len(dataset_test) % num_tasks != 0:
                print('Warning: test set not divisible by #processes')
    ...
    # mixup_fn = None
    # if args.mixup > 0:
    #     print("Multilabel mixup is activated!")
    #     mixup_fn = multilabel_mixup
    mixup_fn = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 10 == 0:
            ...
            num_batches_to_check = 10  # 查看前几个batch

            print("🔍 检查 WeightedRandomSampler 输出的标签分布（前 %d 个 batch）" % num_batches_to_check)

            for i, (images, targets) in enumerate(data_loader_train):
                labels_per_batch = targets.cpu().numpy()
                summed = labels_per_batch.sum(axis=0)  # 多标签时每列求和
                for idx, count in enumerate(summed):
                    label_counter[idx] += int(count)
                if i + 1 >= num_batches_to_check:
                    break

            print("前 %d 个 batch 中采样的标签总计：" % num_batches_to_check)
            for k in sorted(label_counter.keys()):
                print(f"  类别 {k}: {label_counter[k]} 个样本")
...
if __name__ == '__main__':
    ...
    criterion = FocalLoss(alpha=1.0, gamma=2.0, reduction='mean')  # gamma=2 是常用推荐值
